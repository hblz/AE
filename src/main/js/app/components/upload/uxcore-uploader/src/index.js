...
import { UploadCore, Events, Status } from '../modules/uxcore-uploadcore'
import { Icon, Progress, message } from 'antd'
const ProgressLine = Progress.Line;

const UxcoreProgress = require('./progress');
...
class Uploader extends React.Component {
    constructor(props) {
        ...
        let that = this;
        const fileFiltered = (file, error) => {
            // 文件被过滤
            if (that.props.onfilefiltered) {
                that.props.onfilefiltered(file, error);
            }
        }

        this.core.on(Events.QUEUE_STAT_CHANGE, statchange);
        this.core.on(Events.QUEUE_FILE_FILTERED, fileFiltered)
        this.stopListen = () => {
            this.core.off(Events.QUEUE_STAT_CHANGE, statchange);
        };
    }
    ...
    render() {
        ...
        return (
            <div className={"kuma-uploader " + (this.props.className || '')}>
                {this.props.disabled ? <div>{children}</div> : <Picker core={this.core}>{children}</Picker>}
                {this.props.tips}
                {!this.props.hideProgress && this.state.total > 0 ? (
                    <div className="kuma-upload-filelist-container" style={{display: this.state.showUploadList ? "block" : "none"}}>
                        <div className="kuma-upload-filelist-header" style={{display: that.state.total > 0 ? "block" : "none"}}>
                            {this.props.titleHeader}
                            <Icon className="kuma-upload-filelist-close" type="cross-circle-o" title="若是存在文件未上传完成，关闭则会取消上传该文件" onClick={that.closeWindowFun} />
                            <Icon className="kuma-upload-filelist-small" type="circle-o-down" onClick={that.foldWindowFun} style={{display: that.state.foldWindow ? "none" : "block"}} />
                            <Icon className="kuma-upload-filelist-big" type="circle-o-up" onClick={that.unfoldWindowFun} style={{display: that.state.foldWindow ? "block" : "none"}} />
                        </div>
                        <div className="kuma-upload-filelist-body" style={{display: that.state.foldWindow ? "none" : "block"}}>
                            <FileList core={this.core} mode="nw" />
                        </div>
                    </div>
                ) : null}
            </div>
        ...
        if (this.props.mode === 'icon') {
            return <div className={"kuma-upload-fileitem status-" + this.state.status}>
                ...
                <div className="filepreview">
                    ...
                    {this.state.status === 'progress' || this.state.status === 'pending' ? <UxcoreProgress percentage={this.state.percentage} /> : null}
                </div>
            ...
            /*return <div className={"kuma-upload-fileitem status-" + this.state.status}>
                ...
                <label className="field-status">
                    <UxcoreProgress percentage={this.state.percentage} />
                ...
                <ProgressLine percent={this.state.percentage} strokeWidth={5} />
            </div>;*/
            /*LanXiu*/
            return <div className={"kuma-upload-fileitem status-" + this.state.status}>
                <label className="field-info">
                    ...
                    {this.state.status !== 'error' && this.state.status !== 'success' ? <i className="kuma-loading" /> : null}
                    {this.state.status === 'success' ? <i className="kuma-upload-fileicon" data-ext={this.file.ext} data-type={this.file.type}/> : null}
                    <span className="filename"><Icon type="paper-clip" />{this.file.name}</span>
                </label>
                <label className="field-status">
                    {this.state.status === 'error' ? <a className="kuma-upload-status status-error">上传失败</a> : null}
                    <a className="kuma-upload-action" onClick={this.onCancel.bind(this)}>删除</a>
                </label>
                <ProgressLine percent={this.state.percentage} strokeWidth={3} />
            </div>;
        } else {
            ...
            return <div className={"kuma-upload-fileitem status-" + this.state.status}>
                ...
                <UxcoreProgress percentage={this.state.percentage} mode="bar"/>
            </div>;
        }
    }
}
